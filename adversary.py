...
from LDP.estimation import GRR_estimated_guess, RAPPOR_estimated_guess, OUE_estimated_guess, OLH_estimated_guess, \
...
for epsilon in epsilon_list:
    ...
    for _ in range(20):
        grr_est_freq = GRR_advance_estimated_guess(users_consumption_value_list, k, epsilon, 3)
        temp_probability_of_guess_grr.append(grr_est_freq)

        rappor_est_freq = RAPPOR_advance_estimated_guess(users_consumption_value_list, k, epsilon, 3)
        temp_probability_of_guess_rappor.append(rappor_est_freq)

        oue_est_freq = OUE_advance_estimated_guess(users_consumption_value_list, k, epsilon, 3)
        temp_probability_of_guess_oue.append(oue_est_freq)

        olh_est_freq = OLH_advance_estimated_guess(users_consumption_value_list, k, epsilon)
